...
const NftListing = () => {
  ...
  const getNftList = async (page: number) => {
    const res = await httpsClient.get(`nfts/list?per_page=30&page=${page}`);
    return res.data;
  };
  const { data, isLoading } = useQuery(["NFT", page], () => getNftList(page), {
    ...
    <>
                  ...
                  <div className="grid grid-cols-2 mt-4 font-semibold">
                    <div>
                      ...
                      <p>
                        Native currency :{" "}
                        {
                          nftDetailsQueryResult.data?.market_cap
                            ?.native_currency
                        }
                      </p>
                    </div>
                    <div>
                      ...
                      <p>
                        USD : {nftDetailsQueryResult.data?.floor_price?.usd}
                      </p>
                      <p>
                        Native currency :{" "}
                        {
                          nftDetailsQueryResult.data?.floor_price
                            ?.native_currency
                        }
                      </p>
                    </div>
      ...
      <div className="flex justify-between items-center px-10">
        <button
          ...
          disabled={page === 1}
          className="py-2 px-4 bg-white rounded-md border border-gray-500 text-gray-800 font-semibold disabled:bg-gray-300 disabled:text-gray-500"
        >
          ← Prev
        </button>
        <button
          onClick={() => setPage((prevState) => prevState + 1)}
          className="py-2 px-4 bg-white rounded-md border border-gray-500 text-gray-800 font-semibold"
        >
          Next →
        </button>
      </div>
...
const NFTCard = (props: CardProps) => {
  ...
  return (
    <Card
      ...
      onClick={onClick}
    >
      <h2 className="text-[50px] text-center">{symbol}</h2>
